...
public class Camera2Fragment extends Fragment
    ...
    private static final int CREATE_PERSON_CAPTURE_MODE = 3;
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        view.findViewById(R.id.face_capture_button).setOnClickListener(this);
        view.findViewById(R.id.object_capture_button).setOnClickListener(this);
        view.findViewById(R.id.create_person_button).setOnClickListener(this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.face_capture_button: {
                capture_mode = FACE_CAPTURE_MODE;
                takePicture();
                break;
            }
...
//                String url = services.UploadByCloudinary(mFile);
                Log.d("link", url);
                if (url != "") {
                    mResult.post(new Runnable() {
                        @Override
                        public void run() {
                            mResult.setText("Identifying...");
                        }
                    });

                    switch (capture_mode) {
                        case FACE_CAPTURE_MODE: {
                            final String result = services.IdentifyPerson(url);
                            mResult.post(new Runnable() {
                                @Override
                                public void run() {
                        ...
                        case OBJECT_CAPTURE_MODE: {
                            ...
                            if(response != null){
                                mResult.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        mResult.setText(response.description.captions.get(0).text);
                                    }
                                });
                            } else{
                                mResult.post(new Runnable() {
                    ...
                    mResult.post(new Runnable() {
                        @Override
                        public void run() {
                            mResult.setText("Identify Failed");
                        }
                    });
                }
